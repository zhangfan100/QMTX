 ...
 * 仅能用于3.4及其以上的版本 之下的版本prepare只支持一个参数 没有integer参数
 */
@Intercepts( {
       @Signature(method = "prepare", type = StatementHandler.class, args = {Connection.class,Integer.class}) })
public class MySqlDataInterceptor implements Interceptor {

    private static Logger logger = LoggerFactory.getLogger(MySqlDataInterceptor.class);
    /**
     * 拦截后要执行的方法
     */
    public Object intercept(Invocation invocation) throws Throwable {
       ...
       if (obj instanceof PageBean) {
    	   ...
    	   {
    		   // 不分页
               logger.info("[SQL]====> {}====>{} ",boundSql.getParameterObject(), boundSql.getSql());
       ...
       logger.info("[ no page SQL]====> {} =====> {} ",boundSql.getParameterObject(),boundSql.getSql());
       return invocation.proceed();
    }
